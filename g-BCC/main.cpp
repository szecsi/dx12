...
// Forward declaration of the Win32 backend's message handler (imgui_impl_win32.cpp);
// declared here (instead of including a header that pulls in windows.h) so we can
// forward every message to it and let ImGui track mouse/keyboard/cursor state.
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);

std::unique_ptr<BccApp> app{ nullptr };
...
LRESULT CALLBACK WindowProcess(HWND windowHandle, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	// Guard against trailing messages Windows delivers after ShutdownImGui()
	// has already destroyed the context (e.g. WM_NCDESTROY).
	...
	// Input arbitration: don't forward mouse/keyboard to the camera while ImGui
	// wants them (e.g. dragging the Render Mode dropdown). Key-up/mouse-up are
	// always forwarded regardless, so a held-key flag can't get stuck if the
	// matching down/up straddled an ImGui interaction.
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR command, _In_ INT nShowCmd) {
	...
	// select your adapter here, prefer NVIDIA
