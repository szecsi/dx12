...
HWND InitWindow(HINSTANCE hInstance) {
	const wchar_t* windowClassName = L"UVMaskClass";
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance,
                      _In_ LPWSTR command,      _In_ INT nShowCmd) {
	...
	swapChainFsDesc.RefreshRate      = DXGI_RATIONAL{ 60, 1 };
	swapChainFsDesc.Windowed         = true;
	swapChainFsDesc.Scaling          = DXGI_MODE_SCALING_CENTERED;
	swapChainFsDesc.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UPPER_FIELD_FIRST;
	...
	DX_API("Failed to create swap chain for HWND")
		dxgiFactory->CreateSwapChainForHwnd(commandQueue.Get(), windowHandle,
		    &swapChainDesc, &swapChainFsDesc, NULL, tempSwapChain.GetAddressOf());
