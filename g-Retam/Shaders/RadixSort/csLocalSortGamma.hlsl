#define SortSig "RootFlags( 0 )," \
                ...
                "DescriptorTable(UAV(u0, numDescriptors=1), UAV(u1, numDescriptors=1), UAV(u2, numDescriptors=1), UAV(u3, numDescriptors=1), UAV(u4, numDescriptors=1), UAV(u5, numDescriptors=1))" 
...
void csLocalSortGamma(uint3 tid : SV_GroupThreadID, uint3 gid : SV_GroupID)
{
    ...
    for (int did = 0; did < groupDivisor; did++)
    {
        ...
        uint key = keys.Load((ik >> 12) << 2); // costs 200 us more than coherent
